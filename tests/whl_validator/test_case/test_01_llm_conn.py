...
def test_llm_response_trace_is_successful(
    llm_provider: str, llm_runtime_factory
) -> None:
    result = _run_ping(llm_provider, llm_runtime_factory)

    # JSONL 中必须出现至少一条非空 assistant_message，证明模型确实返回内容。
    assistant_messages = [
        event for event in result.traces if event.get("type") == "assistant_message"
    ]
    assert assistant_messages, f"未捕获 assistant_message，traces={result.traces!r}"
    assert any(
        str(event.get("content") or "").strip() for event in assistant_messages
...
@pytest.mark.llm
@pytest.mark.parametrize("llm_provider", LLM_PROVIDERS)
def test_llm_session_has_no_runtime_errors(
    llm_provider: str, llm_runtime_factory
) -> None:
    result = _run_ping(llm_provider, llm_runtime_factory)
