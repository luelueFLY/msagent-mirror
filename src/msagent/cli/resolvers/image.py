...
class ImageResolver(Resolver):
    ...
    @staticmethod
    async def _get_image_files(working_dir: Path, limit: int | None = None, pattern: str = "") -> list[str]:
        ...
        git_globs = " ".join(quote(f"*.{ext.lstrip('.')}") for ext in SUPPORTED_IMAGE_EXTENSIONS)
        fd_flags = " ".join(f"-e {ext.lstrip('.')}" for ext in SUPPORTED_IMAGE_EXTENSIONS)
        ...
        commands = [
            ...
            # fd-based search
            (f"fd --type f {fd_flags} -i {safe_pattern} | {head}" if pattern else f"fd --type f {fd_flags} | {head}"),
        ]
    ...
    async def complete(self, fragment: str, ctx: dict, limit: int) -> list[Completion]:
        ...
        try:
            images = await self._get_image_files(working_dir, limit=limit, pattern=fragment)
