...
def get_latest_features() -> list[str]:
    ...
    try:
        features_yaml = (
            importlib.resources.files("resources").joinpath("features/notes.yml").read_text(encoding="utf-8")
        )
        ...
        for version_key in version_keys:
            ...
            if tuple(int(x) for x in version_minor.split(".")) <= tuple(int(x) for x in current_minor.split(".")):
