...
def format_tool_response(data: Any) -> tuple[str, str | None]:
    ...
    if isinstance(data, (list, tuple)):
        ...
        formatted_dict = {key: format_tool_response(value)[0] for key, value in data.items()}
...
def create_tool_message(
    ...
    final_is_error = is_error if is_error is not None else getattr(result, "is_error", False)
    final_return_direct = return_direct if return_direct is not None else getattr(result, "return_direct", False)
    ...
    for line in diff_lines:
        if line.startswith("@@"):
            ...
            if match:
                ...
                adjusted_line = f"@@ -{old_start},{old_count} +{new_start},{new_count} @@{suffix}"
...
def format_diff_rich(diff_lines: list[str]) -> str:
    ...
    for line in diff_lines:
        if line.strip() == "...":
            formatted_lines.append(f"[{theme.context_color}]...[/{theme.context_color}]")
        elif line.startswith(("---", "+++")):
            continue
        elif m := re.match(r"@@ -(\d+)(?:,\d+)? \+(\d+)", line):
            old_num, new_num = int(m.group(1)), int(m.group(2))
        elif line.startswith("-"):
            formatted_lines.extend(_wrap_diff_line(line[1:], "-", theme.deletion_color, old_num, width, term_width))
            old_num += 1
        elif line.startswith("+"):
            formatted_lines.extend(_wrap_diff_line(line[1:], "+", theme.addition_color, new_num, width, term_width))
            new_num += 1
        elif line.startswith(" "):
            formatted_lines.extend(_wrap_diff_line(line[1:], " ", theme.context_color, old_num, width, term_width))
            old_num += 1
            new_num += 1
