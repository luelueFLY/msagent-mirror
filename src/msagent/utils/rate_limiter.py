...
class TokenBucketLimiter(BaseRateLimiter):
    ...
    def __init__(
        ...
        self.input_token_bucket: float = float(self.max_bucket_size * 100)  # Allow more token burst
        self.output_token_bucket: float = float(self.max_bucket_size * 100)  # Allow more token burst
        ...
        self.recent_input_tokens: deque[int] = deque(maxlen=10)  # Track last 10 input token counts
        self.recent_output_tokens: deque[int] = deque(maxlen=10)  # Track last 10 output token counts
        ...
        with self._consume_lock:
            ...
            if (
                ...
                if input_tokens > 500 or output_tokens > 500:
                    avg_input = sum(self.recent_input_tokens) / max(len(self.recent_input_tokens), 1)
                    logger.info(
