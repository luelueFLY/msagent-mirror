...
def _migrate_items(items: list[dict], config_class: type[VersionedConfig], file_path: Path) -> tuple[list[dict], bool]:
    ...
    if needs_save:
        logger.warning(f"Migrating {config_class.__name__} to version {latest_version}: {file_path}")
...
def _validate_no_duplicates(items: list[dict], key: str, config_type: str) -> None:
    ...
    for idx, item in enumerate(items):
        if key not in item:
            raise ValueError(f"Config item at index {idx} missing required key '{key}': {item}")
        value = item[key]
        if value in seen:
            raise ValueError(
...
async def _load_dir_items(
    ...
    for yml_file in yml_files:
        ...
        if config_class:
            migrated_items, needs_save = _migrate_items(file_items, config_class, yml_file)

            if needs_save:
                save_data = migrated_items if is_list else migrated_items[0]
                yaml_str = yaml.dump(save_data, default_flow_style=False, sort_keys=False)
...
async def _load_single_file(file_path: Path, key: str, config_class: type[VersionedConfig]) -> list[dict]:
    """Load and migrate config items from single file."""
    yaml_content = await asyncio.to_thread(file_path.read_text, encoding=TEXT_ENCODING)
...
async def load_prompt_content(base_path: Path, prompt: str | list[str] | None) -> str | None:
    ...
    if isinstance(prompt, str):
        ...
        if prompt_path.exists() and prompt_path.is_file():
            return await asyncio.to_thread(prompt_path.read_text, encoding=TEXT_ENCODING)
        return prompt
    ...
    if isinstance(prompt, list):
        ...
        for prompt_file in prompt:
            ...
            if prompt_path.exists() and prompt_path.is_file():
                content = await asyncio.to_thread(prompt_path.read_text, encoding=TEXT_ENCODING)
